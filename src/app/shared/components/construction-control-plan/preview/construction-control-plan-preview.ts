...
import {ConstructionControlPlanPoint, ConstructionControlPlanPointConstant} from "../../../../pojos/construction-control-plan/construction-control-plan-point";
...
export class ConstructionControlPlanPreview implements OnDestroy {
  ...
  initOpc(callBack?: (res: any) => void): void {
    ...
    this.http.post('/api/backstage/constructionControlPlan/initLocationsByConstructionControlPlanId', null, params).subscribe((res) => {
      ...
      else if (this.stationId && StringUtils.arrayNotEmpty(this.kilometerMarks))
        centerLocation = this.kilometerMarks.filter(x => x.stationId == this.stationId)[0].locations[0];
      ...
      this.map.panTo(latLng(centerLocation!.latitude, centerLocation!.longitude));

      // 画完整电缆线
      let opcLines: Polyline<turf.helpers.LineString | turf.helpers.MultiLineString, any>[] = [];
      this.rawOpcs.forEach(rawOpc => {
        // let opcType = this.opcTypeMap.get(rawOpc.opcTypeId);
        // let color = opcType ? opcType.color : 'red';
        let color = 'gray';

        if (StringUtils.arrayEmpty(rawOpc.locations)) return;
        this.drawPolyLine(rawOpc, {color: color, weight: 6});
      })
      ...
      setTimeout(() => {
        ...
        this.planOpcs.forEach(opc => {
          ...
          let polyline = this.drawPolyLine(opc, {color: color, weight: 6});

          // 调试代码
          // let i = 0;
          // opc.locations.forEach(location => {
          //   if (i++ % 100 != 0) return;
          //   this.drawMarker(location.longitude, location.latitude, ['233'], undefined, location.id);
          // })
        ...
        if (this.opcMarks) {
          this.opcMarks.forEach(opcMark => {
            if (!opcLines || opcLines.length == 0) return;
            let location = opcMark.locations ? opcMark.locations[0] : undefined;

            let hideMarks = true;
            opcLines.forEach(value => {
              // @ts-ignore
              let distance = turf.pointToLineDistance(turf.point([location?.longitude, location?.latitude]), value.toGeoJSON());
              if (distance < 0.005) {
                hideMarks = false;
                return;
              }
            ...
            if (opcMark.kilometerMark)
              title.push('公里标:' + opcMark.kilometerMark + '');
      ...
      // 画被截取后的铁路线
      if (this.constructionControlPlan)
        this.drawCutRailwayLineSections(this.controlPlanRailwayLineSections, this.constructionControlPlan.startKilometer, this.constructionControlPlan.endKilometer, "方案", 8);
      if (this.constructionDailyPlan)
        this.drawCutRailwayLineSections(this.dailyPlanRailwayLineSections, this.constructionDailyPlan.startKilometer, this.constructionDailyPlan.endKilometer, "日计划", 2);
      ...
      if (this.kilometerMarks) {
        this.kilometerMarks.forEach(kilometerMark => {
          ...
          let customIcon = this.createIcon("/assets/tmp/img/opc-marker.png");
          if (location)
            this.drawMarker(location.longitude, location.latitude, title, customIcon);
      ...
      // 施工点实时位置
      if (this.drawCurrentEquipmentLocationInterval)
    ...
    this.http.post('/api/backstage/equipment/getEquipmentLocationByConstructionControlPlanId', null, params).subscribe((res) => {
      ...
      if (StringUtils.arrayNotEmpty(this.equipments)) {
        this.equipments.forEach(equipment => {
          ...
          let infos = ['设备名：' + equipment.name, '设备编号：' + equipment.imei, '距离光电缆：' + equipment.distanceToOpc?.toFixed(2) + '米'];
          ...
          this.drawMarker(
            lastLocation.longitude, lastLocation.latitude, infos, this.createIcon(imgUrl, [10, 30]), undefined, this.equipmentsLayers
          );
  ...
  toggleMapView(container: HTMLElement) {
    ...
    container.style.backgroundImage = this.showSatelliteMapView ? "url(/assets/map/images/normal-map.png)" : "url(/assets/map/images/satellite.png)";
  ...
  drawRailwayLines(locations: Location[]) {
    ...
    if (!this.map) return;
    // @ts-ignore
    this.baseLayers.push(L.geoJSON(flightsWE, {
      weight: 5,
      color: 'gray',
      fillColor: 'white',
      dashArray: '13, 13',
  ...
  drawConstructionControlPlanPointsInfluenceArea(): void {
    ...
    this.constructionControlPlanPoints.forEach(value => {
      ...
      if (value.collectType == this.constructionControlPlanPointConstant.POINT) {
        ...
        this.addBufferDifference(marker.toGeoJSON(), this.side200Buffer, value.radius, "#00f082")
      }
      ...
      if (value.collectType == this.constructionControlPlanPointConstant.LINE) {
        ...
        this.addBufferDifference(polyline.toGeoJSON(), this.side200Buffer, value.radius, "#00f082")
      }
      ...
      if (value.collectType == this.constructionControlPlanPointConstant.AREA) {
        ...
        this.addBufferDifference(polygon.toGeoJSON(), this.side200Buffer, value.radius, "#00f082")
      }
    })
  }
   ...
   */
  drawPerpendicular(first: Location, second: Location, polyLineOptions: PolylineOptions, markerIcon: Icon, markerTitle: string[], pointOnRight?: boolean) {
    // 计算方位角
  ...
  moveCenter() {
    ...
    this.rawOpcs.forEach(rawOpc => {
      rawOpc.locations.forEach(location => {
        ...
        let marker = this.drawMarker(location.longitude, location.latitude, ['光电缆测量点'], this.createIcon("/assets/map/images/opc-location.png"), undefined, this.locationPointLayers);

        marker.on('click', () => {
          this.checkedLocation = {locationId: location.id, containsLocationData: rawOpc, marker: marker, areOpc: true, areRailwayLineSection: false};
    ...
    this.rawRailwayLineSections.forEach(rawRailwayLineSection => {
      rawRailwayLineSection.locations.forEach(location => {
        ...
        let marker = this.drawMarker(
          location.longitude, location.latitude, ['铁路测量点'], this.createIcon("/assets/map/images/railway-line-location.png"), undefined, this.locationPointLayers
        );
  ...
  drawPopup(lng: number, lat: number, title: string[], layers?: Layer[]) {
    if (layers)
      L.popup().setLatLng(latLng(lat, lng)).setContent(title.join("<br/>")).addTo(new LayerGroup(layers));
   ...
   */
  addBuffer(geoJson: geojson.Feature, outerWidth: number, color: string, innerWidth?: number): { outerBuffer: turf.helpers.Feature<turf.helpers.Polygon | turf.helpers.MultiPolygon, turf.helpers.Properties>, sideBuffer: turf.helpers.Feature<turf.helpers.Polygon | turf.helpers.MultiPolygon, turf.helpers.Properties> } {
  ...
  addBufferWithFillOpacity(geoJson: geojson.Feature, outerWidth: number, color: string, innerWidth: number): turf.helpers.Feature<turf.helpers.Polygon | turf.helpers.MultiPolygon, turf.helpers.Properties> {
    let buffer = turf.buffer(geoJson, outerWidth, {units: 'meters'});
    let featureData;
    let innerBuffer = turf.buffer(geoJson, innerWidth, {units: 'meters'});
    // @ts-ignore
  ...
  createIcon(url: string, iconAnchor?: number[], iconSize?: number[]): Icon {
    ...
    if (iconAnchor)
      iconOptions.iconAnchor = L.point({x: iconAnchor[0], y: iconAnchor[1]});
    if (iconSize)
      iconOptions.iconSize = L.point({x: iconSize[0], y: iconSize[1]});
   ...
   * @param msgPrefix
   */
  drawCutRailwayLineSections(railwayLineSections: RailwayLineSection[], startKilometer: number, endKilometer: number, msgPrefix: string, perpendicularWeight: number): void {
    railwayLineSections.forEach(railwayLineSection => {
      let locations = railwayLineSection.locations;

      this.drawRailwayLines(locations);
      // let imageOverlay = L.imageOverlay("/assets/tmp/img/opc-marker.png", L.latLngBounds(latLngs));
      // this.overlays.push(imageOverlay);

      // 计划
      if (this.constructionControlPlan.railwayLineStatus != railwayLineSection.downriver) return;

      let startKilometerStr = new KilometerPipe().transform(startKilometer);
      let endKilometerStr = new KilometerPipe().transform(endKilometer);
      let startKilometerMsg = [msgPrefix + '起始：' + startKilometerStr];
      let endKilometerMsg = [msgPrefix + '截止：' + endKilometerStr];
      if (locations.length > 2)
        this.drawPerpendicular(locations[0], locations[1], {
          color: '#905a3d',
          weight: perpendicularWeight
        }, this.createIcon("/assets/tmp/img/start-kilometer-marker.png"), startKilometerMsg, false);
      if (locations.length > 4)
        this.drawPerpendicular(locations[locations.length - 1], locations[locations.length - 2], {
          color: '#905a3d',
          weight: perpendicularWeight
        }, this.createIcon("/assets/tmp/img/end-kilometer-marker.png"), endKilometerMsg, true);
    })
  }
}
